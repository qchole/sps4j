...
/**
 * A custom {@link WebServer} that integrates a plugin's Tomcat context with the main application's Tomcat instance.
 * <p>
 * This class is largely a specialization of {@link TomcatWebServer} and is designed to work
 * within the SPS4J plugin framework. It manages the lifecycle of a plugin's web context,
 * ensuring it starts and stops correctly without interfering with the base server.
 * It uses a custom thread to await server stop requests, preventing the plugin from shutting down prematurely.
 *
 * @see Sps4jTomcatWebServerFactory
 */
public class Sps4jTomcatWebServer implements WebServer {


    private static final Log logger  = LogFactory.getLog(Sps4jTomcatWebServer.class);
    ...
    /**
     * Create a new {@link Sps4jTomcatWebServer} instance.
     * @param tomcat the underlying Tomcat server from the base application.
     */
    public Sps4jTomcatWebServer(Tomcat tomcat) {
        this(tomcat, true);
    }
     ...
     */
    public Sps4jTomcatWebServer(Tomcat tomcat, boolean autoStart) {
    ...
    private void addInstanceIdToEngineName() {
        ...
        if (instanceId > 0) {
            // We already have a tomcat container, so just return the existing tomcat.
